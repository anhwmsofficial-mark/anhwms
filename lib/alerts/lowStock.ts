...
type AlertSettingsRow = Database['public']['Tables']['alert_settings']['Row'];
...
export async function checkLowStock(db: SupabaseClient<Database>) {
  ...
  const roleFilter = (settings.notify_roles || []).map((r) => `role.eq.${r}`).join(',');
  ...
  const userIds = Array.from(new Set([...(roleUserRows || []).map((u) => u.id), ...(settings.notify_users || [])]));
  ...
  const externalChannels = (settings.channels || []).filter((c) => c !== 'notification');
