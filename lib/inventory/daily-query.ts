...
export type InventoryDailyDbLike = { from: (table: string) => any };
...
export type InventoryDailySelectOption = {
  id: string;
  name: string;
  code?: string | null;
...
export type InventoryDailyMovementMeta = {
  movementTypes: Array<{
    type: string;
    label: string;
    direction: 'IN' | 'OUT' | null;
...
export type InventoryDailyRowsResult = {
  date: string;
...
export type InventoryDailyMetaResult = InventoryDailyMovementMeta & {
  customers: InventoryDailySelectOption[];
  templates: InventoryDailySelectOption[];
  warehouses: InventoryDailySelectOption[];
};
  ...
  return Boolean(
    error?.message &&
      (error.message.includes(`relation "${relationName}" does not exist`) ||
        error.message.includes(`relation '${relationName}' does not exist`) ||
        error.message.includes(`Could not find the table 'public.${relationName}' in the schema cache`))
  );
}

export function isRecoverableInventorySetupError(error: unknown) {
...
export function safeDailyFallbackDate(request: NextRequest) {
  try {
...
export function buildInventoryDailyMovementMeta(): InventoryDailyMovementMeta {
  return {
...
async function loadStockMap(db: InventoryDailyDbLike, productIds: string[]) {
...
async function fetchSnapshots(db: InventoryDailyDbLike, orgId: string, productIds: string[], date: string) {
  ...
  for (const attempt of attempts) {
    ...
    if (!currentError && !previousExactError) {
      ...
      let previousHistoricalError: { message?: string } | null = null;
      ...
      if (missingPreviousProductIds.length > 0) {
        ...
        if (previousHistoricalResult.error) {
          previousHistoricalError = previousHistoricalResult.error;
        } else {
      ...
      lastError = previousHistoricalError;
    } else {
      lastError = currentError || previousExactError;
    }
...
async function fetchTransactionsForDaily(db: InventoryDailyDbLike, orgId: string, productIds: string[], date: string) {
...
async function loadCustomersForDaily(db: InventoryDailyDbLike, orgId: string) {
...
async function loadWarehousesForDaily(db: InventoryDailyDbLike, orgId: string) {
  const primary = await db
    .from('warehouse')
    .select('id, name')
    .eq('org_id', orgId)
...
export async function loadInventoryDailyMeta(db: InventoryDailyDbLike, orgId: string): Promise<InventoryDailyMetaResult> {
...
export async function loadInventoryDailyRows(
  db: InventoryDailyDbLike,
  orgId: string,
  params: { date: string; search: string; customerId: string | null }
): Promise<InventoryDailyRowsResult> {
  const { date, search, customerId } = params;
  const products = await queryProducts(db, orgId, customerId, search);
  ...
  if (productIds.length > 0) {
    try {
      const snapshotResult = await fetchSnapshots(db, orgId, productIds, date);
      snapshots = snapshotResult.snapshots;
      previousSnapshots = snapshotResult.previousSnapshots;
    } catch (error) {
      warningMessages.push(`스냅샷 조회 fallback: ${error instanceof Error ? error.message : String(error || 'unknown error')}`);
    }

    try {
      transactions = await fetchTransactionsForDaily(db, orgId, productIds, date);
    } catch (error) {
      warningMessages.push(`원장 조회 fallback: ${error instanceof Error ? error.message : String(error || 'unknown error')}`);
    }
  }
  ...
  // Keep the legacy row calculation identical while the PR only changes fetch UX.
