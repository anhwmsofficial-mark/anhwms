...
export default function ExportTemplateManagerClient({ initialData, initialError }: Props) {
  ...
  const [selectedTemplateId, setSelectedTemplateId] = useState<string>(initialData.templates[0]?.id || 'new');
  ...
  return (
    <div className="flex h-screen flex-col bg-gray-50">
      <Header title="템플릿 UI" backUrl="/inventory/management" />
            ...
            <div className="min-h-0 flex-1 overflow-y-auto px-6 py-5">
              {initialData.setupRequired ? (
                <div className="mb-5 rounded-xl border border-amber-200 bg-amber-50 px-4 py-4 text-sm text-amber-800">
                  템플릿 저장용 테이블이 아직 준비되지 않았습니다. 화면 구성은 먼저 확인할 수 있으며, 실제 저장은 마이그레이션 적용 후 가능합니다.
                </div>
              ) : null}
              ...
              <div className="mb-6 grid gap-3 xl:grid-cols-3">
                <div className="rounded-xl border border-blue-100 bg-blue-50 px-4 py-3">
                  ...
                  <p className="mt-1 text-xs leading-5 text-blue-800">
                    업체별 제출 포맷을 분리해 관리하고, 필요한 트랜잭션 컬럼만 골라 저장할 수 있습니다.
                  </p>
                </div>
                <div className="rounded-xl border border-emerald-100 bg-emerald-50 px-4 py-3">
                  <div className="text-sm font-semibold text-emerald-900">사용법 2</div>
                  <p className="mt-1 text-xs leading-5 text-emerald-800">
                    헤더명과 순서를 바꾸면 업체 제출 엑셀의 표시 순서도 동일하게 맞출 수 있습니다.
                  </p>
                </div>
                <div className="rounded-xl border border-gray-200 bg-gray-50 px-4 py-3">
                  <div className="text-sm font-semibold text-gray-900">다음 단계</div>
                  <p className="mt-1 text-xs leading-5 text-gray-700">
                    저장 구조가 안정화되면 이후 `Export`와 `Inventory UI`에 연결하는 순서로 확장합니다.
                  </p>
                </div>
              </div>
              ...
              <div className="grid gap-4 lg:grid-cols-2">
                <label className="flex flex-col gap-1">
                  ...
                  <input
                    ...
                    placeholder="예: inventory-default"
                  />
                </label>
              ...
              <div className="mt-8 grid gap-6 xl:grid-cols-[minmax(0,1.3fr)_360px]">
                            ...
                            <tr key={column.key}>
                              ...
                              <td className="px-3 py-3">
                                <input
                                  ...
                                  onChange={(event) => handleColumnChange(column.key, { displayName: event.target.value })}
                            ...
                            <tr key={column.key} className={column.enabled ? 'bg-white' : 'bg-gray-50/50'}>
                              <td className="px-3 py-3 text-center">
                                <input
                                  ...
                                  onChange={(event) => handleColumnChange(column.key, { enabled: event.target.checked })}
                ...
                <aside className="rounded-2xl border border-gray-200 bg-gray-50 p-5">
                  ...
                  <p className="mt-1 text-xs text-gray-500">현재 설정 기준으로 엑셀에 노출될 컬럼 순서입니다.</p>
                  ...
                  <div className="mt-6 rounded-xl border border-dashed border-gray-300 bg-white px-4 py-3 text-xs text-gray-600">
                    저장 구조가 준비되면 이 설정이 실제 엑셀 Export와 연결됩니다.
                    <Link href="/inventory/management" className="mt-2 block font-medium text-blue-600 hover:text-blue-700">
                      재고 관리로 돌아가기
                    </Link>
                  </div>
                </aside>
