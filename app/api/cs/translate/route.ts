...
- 결과물만 출력하고 설명은 포함하지 마십시오.`;
      ...
      // 중국어 타겟 프롬프트 (Chinese Target Prompt - 한->중 번역 품질 개선)
...
## 翻译详细指南（非常重要）：
1. **时间表达**：
   - 翻译"到明天为止"、"明天前"等表达时，应根据语境判断。如果是"在明天之内完成"，请使用"明天内(明天内)"或"明天(明天)"，避免使用"明天之前(明天之前)"，因为这可能被误解为"今天之内"。
   - 示例："내일까지 전달드릴게요" -> "明天会传达给您" 或 "明天内会给您答复" (O), "明天之前..." (X - 容易产生歧义)

2. **委婉表达 (商务语境)**：
   - 翻译"어렵습니다(困难)"时，在商务语境下应更加委婉。不要直接翻译为"很困难(很困难)"。
   - 推荐表达："恐怕很难处理(아마 처리하기 어려울 것 같습니다)"、"不太方便(조금 불편합니다/어렵습니다)"、"暂时无法满足(당장은 충족시키기 어렵습니다)"。
   - 示例："오늘은 출고량이 많아 어렵습니다" -> "今天出库量较大，恐怕很难处理"

3. **常用商务句型**：
   - "알고 있습니다(知道)" -> 在确认业务内容时，使用"据了解(파악하기로는)"或"我方已知悉(저희측도 인지했습니다)"更为专业。
   - 示例："해당건은... 알고있습니다" -> "关于该事项... 据我方了解..."
